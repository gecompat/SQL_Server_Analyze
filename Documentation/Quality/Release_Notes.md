...
- Der synthetische `EV-SEVERE`-XE-Vertrag verwendet den für `error_reported` zulässigen Single-Event-Loss-Modus, eine Sekunde Dispatch-Latenz, mehrere kontrollierte Ereignisse und wartet vor dem Stop auf die Dateiausgabe. Dadurch kann weder ein einzelner zulässig verlorener Event noch ein asynchroner Flush den Nachweis zufällig kippen.
...
- Die achtzehnte Suite automatisiert fehlende Full-Evidenz, eine nicht mehr zur neuesten Fullbasis passende Differentialzeile, eine kontrolliert sichtbare Logkettenlücke und fehlende Restorehistorie. Sie verwendet nur die synthetische Testdatenbank und das plattformspezifische Nullgerät, bereinigt die kurzlebige `msdb`-Historie, stellt das Recovery Model wieder her und führt bewusst keinen Restore aus; die vier Statuszeilen bleiben bis zu grünen Actions unverändert.
