...
Der Stand `1.1.0-special.9` besitzt vollständige grüne P0-Evidenz. Der Release-Gate-Vertrag ist um eine fünfzehnte, versionsadaptive P1-IQP-Suite erweitert; ihre vier Matrixfälle bleiben bis zu grünen commitbezogenen SQL-Server-2019-/2022-/2025-Läufen `NOT_EXECUTED`.
...
27. P1 in Reihenfolge begonnen: `170_P1_IQP_Runtime_Contract.sql` prüft SQL-Server-2019-Abgrenzung, PSP auf Compatibility Level 160, OPPO auf Level 170 und Query Store OFF mit rücksetzbaren Datenbankoptionen. Eine Statusänderung erfolgt erst nach grünen Drei-Versionen-Gates.
...
1. Pro manuellem Ziel `Code/Tests/Run_Release_Gate.sql` im SQLCMD-Modus aus `Code/Tests` ausführen; der Runner startet `110`, `163`, `165`, `167`, `168`, `169` und `170` sowie acht Bereichssuiten in fester Reihenfolge und bricht beim ersten SQL-Fehler ab.
