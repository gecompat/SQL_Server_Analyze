...
IF NOT EXISTS(SELECT 1 FROM [sys].[columns] AS [c] WITH (NOLOCK) JOIN [sys].[tables] AS [t] WITH (NOLOCK) ON [t].[object_id]=[c].[object_id] JOIN [sys].[schemas] AS [s] WITH (NOLOCK) ON [s].[schema_id]=[t].[schema_id] WHERE [s].[name]=N'monitor' AND [t].[name]=N'RelatedWaitTypes') ALTER TABLE [monitor].[WaitTypeCatalog] ADD [RelatedWaitTypes] nvarchar(1000) NULL;
IF NOT EXISTS(SELECT 1 FROM [sys].[columns] AS [c] WITH (NOLOCK) JOIN [sys].[tables] AS [t] WITH (NOLOCK) ON [t].[object_id]=[c].[object_id] JOIN [sys].[schemas] AS [s] WITH (NOLOCK) ON [s].[schema_id]=[t].[schema_id] WHERE [s].[name]=N'monitor' AND [t].[name]=N'MeasurementGuidance') ALTER TABLE [monitor].[WaitTypeCatalog] ADD [MeasurementGuidance] nvarchar(1500) NOT NULL CONSTRAINT DF_WaitTypeCatalog_MeasurementGuidance DEFAULT(N'Aktive Tasks und ein belastungsbezogenes Delta gemeinsam bewerten; kumulative Werte seit Start oder Reset sind allein kein Befund.') WITH VALUES;
IF NOT EXISTS(SELECT 1 FROM [sys].[columns] AS [c] WITH (NOLOCK) JOIN [sys].[tables] AS [t] WITH (NOLOCK) ON [t].[object_id]=[c].[object_id] JOIN [sys].[schemas] AS [s] WITH (NOLOCK) ON [s].[schema_id]=[t].[schema_id] WHERE [s].[name]=N'monitor' AND [t].[name]=N'AnalysisConfidence') ALTER TABLE [monitor].[WaitTypeCatalog] ADD [AnalysisConfidence] varchar(30) NOT NULL CONSTRAINT DF_WaitTypeCatalog_AnalysisConfidence DEFAULT('FAMILY_INFERENCE') WITH VALUES;
GO
