...
BEGIN
    ...
    IF @StatusCode='AVAILABLE' EXEC [monitor].[USP_PrepareDatabaseCandidates] @DatabaseNames=@QueryStoreDatabaseNames,@SystemdatenbankenEinbeziehen=0,@DatabaseNamePattern=@QueryStoreDatabaseNamePattern,@MaxDatenbanken=@MaxDatenbanken,@AnalysisClass='CROSS_DATABASE_DEEP',@StatusCode=@StatusCode OUTPUT,@ErrorMessage=@ErrorMessage OUTPUT,@CrossDatabaseRequested=@Cross OUTPUT;
    ...
    BEGIN
      DECLARE [c] CURSOR LOCAL FAST_FORWARD FOR SELECT [DatabaseName],[CompatibilityLevel] FROM [#DatabaseCandidates] ORDER BY COALESCE([RequestedOrdinal],[DatabaseId]),[DatabaseId];OPEN [c];FETCH NEXT FROM [c] INTO @Db,@DbCompat;
      WHILE @@FETCH_STATUS=0
      BEGIN
        IF @RefMode IN('REGEX','REGEXI') AND (TRY_CONVERT(int,SERVERPROPERTY(N'ProductMajorVersion'))<17 OR @DbCompat<170) BEGIN INSERT [#Errors] VALUES(@Db,'UNAVAILABLE_FEATURE',NULL,N'Regex benötigt SQL Server 2025 und Compatibility Level 170.');SET @IsPartial=1;END
        ELSE BEGIN TRY
          SET @Sql=N'USE '+QUOTENAME(@Db)+N';IF EXISTS(SELECT 1 FROM [sys].[database_query_store_options] WHERE [actual_state] IN(1,2,4) AND [wait_stats_capture_mode]=1)
BEGIN
;WITH [W] AS
(
 ...
 FROM [sys].[query_store_wait_stats] [ws] JOIN [sys].[query_store_runtime_stats_interval] [i] ON [i].[runtime_stats_interval_id]=[ws].[runtime_stats_interval_id]
...
INSERT [#Result]
SELECT TOP(@TopRows) DB_ID(),DB_NAME(),[q].[query_id],[p].[plan_id],[q].[query_hash],[p].[query_plan_hash],[W].[wait_category],[W].[wait_category_desc],[W].[execution_type_desc],[W].[FirstStart],[W].[LastEnd],[W].[RecordedRows],[W].[TotalWait],CONVERT(decimal(38,3),[W].[AverageWait]),[W].[MaxWait],CASE WHEN @TextChars IS NULL OR @TextChars=0 THEN [qt].[query_sql_text] ELSE LEFT([qt].[query_sql_text],@TextChars) END
FROM [W] JOIN [sys].[query_store_plan] [p] ON [p].[plan_id]=[W].[plan_id] JOIN [sys].[query_store_query] [q] ON [q].[query_id]=[p].[query_id] JOIN [sys].[query_store_query_text] [qt] ON [qt].[query_text_id]=[q].[query_text_id]
...
END;';
    ...
    END;
    SELECT @RowCount=COUNT_BIG(*) FROM [#Result];SET @HasMoreRows=CONVERT(bit,CASE WHEN @EffectiveMaxZeilen<9223372036854775807 AND @RowCount>@EffectiveMaxZeilen THEN 1 ELSE 0 END);IF @IsPartial=1 AND @StatusCode='AVAILABLE' SET @StatusCode='AVAILABLE_LIMITED';
