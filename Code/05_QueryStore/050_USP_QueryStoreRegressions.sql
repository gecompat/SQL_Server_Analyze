...
INSERT [#Result] SELECT TOP(@TopRows) DB_ID(),DB_NAME(),[query_id],[query_hash],[object_id],CASE WHEN [P].[object_id]>0 THEN QUOTENAME([os].[name])+N''.''+QUOTENAME([oo].[name]) END,[bexec],[cexec],[bplans],[cplans],CONVERT(decimal(38,3),[bvalue]),CONVERT(decimal(38,3),[cvalue]),CONVERT(decimal(38,3),[cvalue]-[bvalue]),CONVERT(decimal(38,3),100.0*([cvalue]-[bvalue])/NULLIF(ABS([bvalue]),0)),[last_execution_time],CASE WHEN @TextChars IS NULL OR @TextChars=0 THEN [query_sql_text] ELSE LEFT([query_sql_text],@TextChars) END FROM [P] LEFT JOIN [sys].[objects] AS [oo] WITH (NOLOCK) ON [oo].[object_id]=[P].[object_id] LEFT JOIN [sys].[schemas] AS [os] WITH (NOLOCK) ON [os].[schema_id]=[oo].[schema_id] WHERE [bvalue] IS NOT NULL AND [cvalue] IS NOT NULL AND 100.0*([cvalue]-[bvalue])/NULLIF(ABS([bvalue]),0)>=@MinPct ORDER BY 100.0*([cvalue]-[bvalue])/NULLIF(ABS([bvalue]),0) DESC,[cvalue]-[bvalue] DESC;
END;';
