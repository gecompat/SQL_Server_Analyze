...
BEGIN
    ...
    ELSE IF @AnalyseModus='GEZIELT'
         AND NOT EXISTS(SELECT 1 FROM [#NameFilters] WHERE [FilterType]='FULL_OBJECT')
         AND (NOT EXISTS(SELECT 1 FROM [#NameFilters] WHERE [FilterType]='SCHEMA')
              OR NOT EXISTS(SELECT 1 FROM [#NameFilters] WHERE [FilterType]='OBJECT'))
    ...
    BEGIN
        INSERT [#DatabaseStatus] VALUES(@DatabaseName,@OverallStatus,1,0,NULL,NULL,@ErrorMessage,N'Keine DMF-Abfrage ausgeführt.');
        SET @IsPartial=1;
    END
    ELSE
    BEGIN
...
OPTION (MAXDOP 1,RECOMPILE);';
    ...
    BEGIN
        ...
        BEGIN SET @OverallStatus=CASE WHEN @TotalRows>0 THEN 'PARTIAL' ELSE (SELECT TOP(1) [StatusCode] FROM [#DatabaseStatus] WHERE [StatusCode] NOT IN ('AVAILABLE','AVAILABLE_LIMITED','SKIPPED','NOT_APPLICABLE') ORDER BY [DatabaseName]) END; SET @IsPartial=1; END
