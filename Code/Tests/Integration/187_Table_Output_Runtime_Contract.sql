...
IF @Status<>'AVAILABLE' OR @Rows<>2
    INSERT [#Failure] VALUES(N'PLACEHOLDER_ADAPT',CONCAT(N'Erwartet AVAILABLE/2, erhalten ',COALESCE(@Status,N'NULL'),N'/',COALESCE(CONVERT(nvarchar(30),@Rows),N'NULL'),N': ',COALESCE(@ErrorMessage,N'')));
...
IF COL_LENGTH(N'tempdb..#TableContractTarget',N'__MonitorPlaceholder') IS NOT NULL
   ...
   OR COL_LENGTH(N'tempdb..#TableContractTarget',N'Name')<>80
    INSERT [#Failure] VALUES(N'NATIVE_COLUMN_SHAPE',N'Platzhalter oder native int-/nvarchar-Struktur stimmt nach der Adaption nicht.');
...
(
    ...
    INSERT [#Failure] VALUES(N'DECIMAL_SHAPE',N'Precision, Scale oder Nullability der decimal-Spalte wurden nicht erhalten.');
...
(
    ...
    INSERT [#Failure] VALUES(N'DATETIME_SHAPE',N'Scale oder Nullability der datetime2-Spalte wurden nicht erhalten.');
...
IF (SELECT COUNT_BIG(*) FROM [#TableContractTarget])<>2
   OR NOT EXISTS(SELECT 1 FROM [#TableContractTarget] WHERE [Id]=1 AND [Name]=N'Example A' AND [Amount]=CONVERT(decimal(19,4),12.3456))
    INSERT [#Failure] VALUES(N'TYPED_INSERT',N'Die synthetischen typisierten Zeilen wurden nicht unverändert kopiert.');
...
IF @Status<>'AVAILABLE' OR @Rows<>2 OR (SELECT COUNT_BIG(*) FROM [#TableContractTarget])<>4
    INSERT [#Failure] VALUES(N'EXACT_SCHEMA_APPEND',N'Eine bereits exakt passende Zieltabelle wurde nicht korrekt ergänzt.');
...
IF @Status<>'TARGET_SCHEMA_MISMATCH'
   ...
   OR COL_LENGTH(N'tempdb..#TableContractMismatch',N'Name') IS NOT NULL
    INSERT [#Failure] VALUES(N'SCHEMA_MISMATCH_SAFE',N'Eine gefüllte abweichende Zieltabelle wurde nicht kontrolliert und unverändert abgelehnt.');
...
IF @Status<>'TARGET_SCHEMA_MISMATCH'
   OR COL_LENGTH(N'tempdb..#TableContractNonEmptyPlaceholder',N'__MonitorPlaceholder') IS NULL
    INSERT [#Failure] VALUES(N'NONEMPTY_PLACEHOLDER_SAFE',N'Ein gefüllter Platzhalter wurde nicht kontrolliert und unverändert abgelehnt.');
...
IF @Status<>'INVALID_PARAMETER'
    INSERT [#Failure] VALUES(N'LOCAL_TEMP_ONLY',N'Ein permanenter Tabellenname wurde nicht abgelehnt.');
...
BEGIN TRY
    ...
    IF COL_LENGTH(N'tempdb..#PublicTableTarget',N'__MonitorPlaceholder') IS NOT NULL
       ...
       OR COL_LENGTH(N'tempdb..#PublicTableTarget',N'StatusCode') IS NULL
        INSERT [#Failure] VALUES(N'PUBLIC_TABLE_MODE',N'Die öffentliche Procedure hat ihre primäre native Ergebnisstruktur nicht in @ResultTable geschrieben.');
END TRY
BEGIN CATCH
    INSERT [#Failure] VALUES(N'PUBLIC_TABLE_MODE',CONCAT(N'Öffentlicher TABLE-Aufruf fehlgeschlagen: ',ERROR_MESSAGE()));
END CATCH;
...
BEGIN TRY
    EXEC [monitor].[USP_CheckAnalyseAccess]
        ...
        , @PrintMeldungen=0;
    INSERT [#Failure] VALUES(N'PUBLIC_SCHEMA_ERROR',N'Die öffentliche Procedure hat eine abweichende Zielstruktur nicht mit Fehler 51010 abgelehnt.');
END TRY
BEGIN CATCH
    IF ERROR_NUMBER()<>51010
        INSERT [#Failure] VALUES(N'PUBLIC_SCHEMA_ERROR',CONCAT(N'Erwartet Fehler 51010, erhalten ',ERROR_NUMBER(),N': ',ERROR_MESSAGE()));
END CATCH;
...
IF COL_LENGTH(N'tempdb..#PublicTableMismatch',N'WrongColumn') IS NULL
   OR COL_LENGTH(N'tempdb..#PublicTableMismatch',N'AnalysisClass') IS NOT NULL
    INSERT [#Failure] VALUES(N'PUBLIC_SCHEMA_UNCHANGED',N'Die öffentliche Procedure hat eine abweichende Zielstruktur verändert.');
