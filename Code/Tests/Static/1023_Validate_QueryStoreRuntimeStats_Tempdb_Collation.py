#!/usr/bin/env python3
"""Validate explicit temp-table collation in Query Store Runtime Stats."""
...
PROCEDURE_PATH = Path("Code/05_QueryStore/020_USP_QueryStoreRuntimeStats.sql")
COLLATION = "COLLATE SQL_Latin1_General_CP1_CS_AS"
TABLE_REQUIREMENTS = {
    "#QueryStoreRuntimeStats_DatabaseCandidates": 5,
    "#QueryStoreRuntimeStats_Result": 10,
    "#QueryStoreRuntimeStats_Errors": 3,
}
...
def main() -> int:
    ...
    if args.self_test:
        print("Query Store Runtime Stats tempdb-collation validator self-test passed.")
        return 0
    ...
    if failures:
        print("Query Store Runtime Stats tempdb-collation validation failed: " + "; ".join(failures))
        return 1

    print("Query Store Runtime Stats tempdb-collation validation passed.")
    return 0
