...
def validate_container_installer(root: Path, findings: list[str]) -> None:
    ...
    require(
        -1 not in (classify, install, verify) and classify < install < verify,
        "Container framework installer does not classify, install, and verify in order.",
        findings,
    )
    installer = text(
        root,
...
def validate_scope_action(root: Path, findings: list[str]) -> None:
    ...
    fragments(
        ...
        (
            ...
            "FRAMEWORK_SCOPE_NOT_READY",
            "FRAMEWORK_SCOPE_CONFLICT",
            "FRAMEWORK_OWNERSHIP_MISMATCH",
            "FRAMEWORK_UPDATE_FAILED",
            "LifecycleStatus -ne 'READY'",
    ...
    require(
        -1 not in (ready_check, status_check, discovery, approval, state, install, final)
        and ready_check < status_check < discovery < approval < state < install < final,
        "UpdateFramework does not validate, confirm, persist, install, and report in order.",
        findings,
    )
    for forbidden in (
...
def validate_tests_and_workflow(root: Path, findings: list[str]) -> None:
    ...
    workflow = text(root, ".github/workflows/quicktest-lifecycle-validation.yml")
    fragments(
        tests,
        (
            "FrameworkAction -ne 'INSTALLED'",
            "FrameworkAction -ne 'UPDATED'",
            "FRAMEWORK_READY",
            "FrameworkUpdateStatus",
            "UpdateFramework changed runtime identity",
            "UpdateFramework recreated the container through Compose",
            "Classify_Framework.sql",
            "Verify_Framework.sql",
        ),
    ...
    fragments(
        workflow,
        (
            "Validate_Docker_Podman_QuickTest_UpdateFramework.py",
            "Invoke-LabQuickTestUpdateFrameworkTests.ps1",
...
def validate_status_and_docs(root: Path, findings: list[str]) -> None:
    ...
    fragments(
        delivered,
        (
            "UpdateFramework action",
            "INSTALLED",
            "UPDATED",
            "per-instance",
            "FRAMEWORK_READY",
        ),
        "Delivered UpdateFramework status",
        findings,
    )
    fragments(
        opened,
        (
            "UpdateFramework execution evidence",
...
def main() -> int:
    ...
    if findings:
        ...
        return 1
    print(
        "Docker/Podman quick-test UpdateFramework validated: "
        "classification=INSTALLED_OR_UPDATED verification=FRAMEWORK_READY "
