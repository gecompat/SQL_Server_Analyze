#!/usr/bin/env python3
"""Validate explicit temp-table collation in Query Store Forced Plans."""
...
PROCEDURE_PATH = Path("Code/05_QueryStore/060_USP_QueryStoreForcedPlans.sql")
...
def main() -> int:
    ...
    if args.self_test:
        print("Query Store Forced Plans tempdb-collation validator self-test passed.")
        return 0
    ...
    if failures:
        print("Query Store Forced Plans tempdb-collation validation failed: " + "; ".join(failures))
        return 1

    print("Query Store Forced Plans tempdb-collation validation passed.")
    return 0
