#!/usr/bin/env python3
"""Validate explicit framework collations in CurrentIO work tables."""
...
TABLE = re.compile(r"CREATE\s+TABLE\s+\[#CurrentIO_[^\]]+\]\s*\((.*?)\);", re.I | re.S)
...
def main() -> int:
    ...
    if args.self_test:
        print("CurrentIO tempdb-collation validator self-test passed.")
        return 0
    ...
    if not definitions or missing:
        print("CurrentIO tempdb-collation validation failed:", file=sys.stderr)
        print("\\n".join(f"- {column}" for column in missing) or "- no work tables found", file=sys.stderr)
        return 1

    print(f"CurrentIO tempdb-collation validation passed: tables={len(definitions)}.")
    return 0
