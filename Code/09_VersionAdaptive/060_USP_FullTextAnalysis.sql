...
BEGIN
    ...
    CREATE TABLE [#SemanticPopulation]
    (
          [DatabaseName] sysname NOT NULL
        , [SchemaName] sysname NOT NULL
        , [TableName] sysname NOT NULL
    ...
    IF @SchemaPatternMode IN('REGEX','REGEXI')
        SET @SchemaPredicate+=N' AND REGEXP_LIKE([s].[name],N'''+REPLACE(@SchemaPatternValue,N'''',N'''''')+N''','''+@SchemaRegexFlags+N''')=1';
    ...
    IF @ObjectPatternMode IN('REGEX','REGEXI')
        SET @ObjectPredicate+=N' AND REGEXP_LIKE([t].[name],N'''+REPLACE(@ObjectPatternValue,N'''',N'''''')+N''','''+@ObjectRegexFlags+N''')=1';
...
INSERT [#FeatureScope]([DatabaseName],[IsFullTextInstalled],[CatalogCount],[FullTextIndexCount],[SemanticColumnCount])
...
SET @pRows=@@ROWCOUNT;';
            ...
            END TRY
            BEGIN CATCH
                DELETE FROM [#Catalog] WHERE [DatabaseName]=@DbName;
                DELETE FROM [#FullTextIndex] WHERE [DatabaseName]=@DbName;
                INSERT [#SourceStatus]
...
FROM [sys].[dm_fts_outstanding_batches] [b]
JOIN [#FullTextIndex] [fi]
...
FROM [sys].[dm_fts_semantic_similarity_population] [p]
JOIN [#FullTextIndex] [fi]
  ON [fi].[DatabaseName]=@pDatabaseName AND [fi].[TableObjectId]=[p].[table_id]
WHERE [p].[database_id]=DB_ID();
SET @pRows=@@ROWCOUNT;';
    ...
    WHERE EXISTS(SELECT 1 FROM [#Population])
      AND NOT EXISTS(SELECT 1 FROM [#FdHost])
      AND EXISTS(SELECT 1 FROM [#SourceStatus] WHERE [SourceCode]='FULLTEXT_FDHOSTS' AND [StatusCode]='AVAILABLE' AND [IsPartial]=0)
      AND NOT EXISTS(SELECT 1 FROM [#SourceStatus] WHERE [SourceCode]='FULLTEXT_POPULATION' AND [IsPartial]=1);
    ...
    BEGIN
        ...
        ELSE IF NOT EXISTS
            (SELECT 1 FROM [#FeatureScope] WHERE [CatalogCount]>0 OR [FullTextIndexCount]>0)
    ...
    BEGIN
        SELECT @Json=(
            SELECT
                ...
                JSON_QUERY(COALESCE((SELECT TOP(@Limit) * FROM [#Catalog] WHERE @NurProblematisch=0 OR [AssessmentStatus]='REVIEW' ORDER BY CASE [AssessmentStatus] WHEN 'REVIEW' THEN 1 ELSE 2 END,[LogicalSizeMb] DESC,[DatabaseName],[CatalogName] FOR JSON PATH),N'[]')) AS [catalogs],
                JSON_QUERY(COALESCE((SELECT TOP(@Limit) * FROM [#FullTextIndex] WHERE @NurProblematisch=0 OR [AssessmentStatus]='REVIEW' ORDER BY CASE [AssessmentStatus] WHEN 'REVIEW' THEN 1 ELSE 2 END,[QueryableFragmentCount] DESC,[DatabaseName],[SchemaName],[TableName] FOR JSON PATH),N'[]')) AS [fullTextIndexes],
                JSON_QUERY(COALESCE((SELECT TOP(@Limit) * FROM [#Population] WHERE @NurProblematisch=0 OR [AgeMinutes]>=@PopulationAgeWarnMinutes OR [Status]=11 ORDER BY [AgeMinutes] DESC,[DatabaseName],[SchemaName],[TableName] FOR JSON PATH),N'[]')) AS [populations],
                JSON_QUERY(COALESCE((SELECT TOP(@Limit) * FROM [#BatchGroup] WHERE @NurProblematisch=0 OR COALESCE([BatchErrorCode],0)<>0 OR [IsRetryBatch]=1 OR [FailedDocumentCount]>=@FailedDocumentWarn ORDER BY [FailedDocumentCount] DESC,[BatchCount] DESC,[DatabaseName],[SchemaName],[TableName] FOR JSON PATH),N'[]')) AS [outstandingBatches],
                JSON_QUERY(COALESCE((SELECT TOP(@Limit) * FROM [#SemanticPopulation] WHERE @NurProblematisch=0 OR [AgeMinutes]>=@PopulationAgeWarnMinutes ORDER BY [AgeMinutes] DESC,[DatabaseName],[SchemaName],[TableName] FOR JSON PATH),N'[]')) AS [semanticPopulations],
                JSON_QUERY(COALESCE((SELECT TOP(@Limit) * FROM [#MemoryPool] ORDER BY [AllocatedMb] DESC,[PoolId] FOR JSON PATH),N'[]')) AS [memoryPools],
                JSON_QUERY(COALESCE((SELECT TOP(@Limit) * FROM [#FdHost] ORDER BY [FdHostType] FOR JSON PATH),N'[]')) AS [fdHosts]
    ...
    BEGIN
        ...
        SELECT TOP(@Limit) * FROM [#FullTextIndex]
        WHERE @NurProblematisch=0 OR [AssessmentStatus]='REVIEW'
        ORDER BY CASE [AssessmentStatus] WHEN 'REVIEW' THEN 1 ELSE 2 END,
                 [QueryableFragmentCount] DESC,[DatabaseName],[SchemaName],[TableName];
        SELECT TOP(@Limit) * FROM [#Population]
        ...
        SELECT TOP(@Limit) * FROM [#BatchGroup]
        ...
        SELECT TOP(@Limit) * FROM [#SemanticPopulation]
        WHERE @NurProblematisch=0 OR [AgeMinutes]>=@PopulationAgeWarnMinutes
        ORDER BY [AgeMinutes] DESC,[DatabaseName],[SchemaName],[TableName];
        SELECT TOP(@Limit) * FROM [#MemoryPool] ORDER BY [AllocatedMb] DESC,[PoolId];
        SELECT TOP(@Limit) * FROM [#FdHost] ORDER BY [FdHostType];
    END;
