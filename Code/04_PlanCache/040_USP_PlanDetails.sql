...
BEGIN
    ...
    BEGIN
        ...
        IF (SELECT COUNT_BIG(*) FROM [#SessionIdFilter])=1 SELECT @SingleSessionId=MIN([SessionId]) FROM [#SessionIdFilter];
    END;
    ...
    CREATE TABLE [#Attributes]([CandidateId] int,[AttributeName] varchar(128),[AttributeValue] nvarchar(4000),[IsCacheKey] bit);
    CREATE TABLE [#Plans]([CandidateId] int,[SourceType] varchar(24),[StatusCode] varchar(40),[DatabaseId] int NULL,[ObjectId] int NULL,[IsEncrypted] bit NULL,[QueryPlanXml] xml NULL,[QueryPlanText] nvarchar(max) NULL,[ErrorNumber] int NULL,[ErrorMessage] nvarchar(2048) NULL);
    ...
    BEGIN TRY
        IF @SessionIds IS NOT NULL
        BEGIN
            INSERT [#Candidate]([SessionId],[RequestId],[PlanHandle],[SqlHandle],[QueryHash],[QueryPlanHash],[StatementStartOffset],[StatementEndOffset],[CreationTime],[LastExecutionTime],[ExecutionCount])
            ...
            FROM [sys].[dm_exec_requests] AS [r] JOIN [#SessionIdFilter] AS [sf] ON [sf].[SessionId]=[r].[session_id] ORDER BY [r].[session_id],[r].[request_id];
        END;
        IF @PlanHandle IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [#Candidate] WHERE [PlanHandle]=@PlanHandle)
            INSERT [#Candidate]([PlanHandle]) VALUES(@PlanHandle);
        IF @SqlHandle IS NOT NULL
            INSERT [#Candidate]([PlanHandle],[SqlHandle],[QueryHash],[QueryPlanHash],[StatementStartOffset],[StatementEndOffset],[CreationTime],[LastExecutionTime],[ExecutionCount])
            ...
            FROM [sys].[dm_exec_query_stats] AS qs WHERE [qs].[sql_handle]=@SqlHandle AND NOT EXISTS(SELECT 1 FROM [#Candidate] c WHERE [c].[PlanHandle]=[qs].[plan_handle] AND COALESCE([c].[StatementStartOffset],-1)=COALESCE([qs].[statement_start_offset],-1))
            ORDER BY [qs].[total_worker_time] DESC;
        IF @QueryHash IS NOT NULL
            INSERT [#Candidate]([PlanHandle],[SqlHandle],[QueryHash],[QueryPlanHash],[StatementStartOffset],[StatementEndOffset],[CreationTime],[LastExecutionTime],[ExecutionCount])
            SELECT TOP (@EffectiveMaxAnalyseobjekte) [qs].[plan_handle],[qs].[sql_handle],[qs].[query_hash],[qs].[query_plan_hash],[qs].[statement_start_offset],[qs].[statement_end_offset],[qs].[creation_time],[qs].[last_execution_time],[qs].[execution_count]
            FROM [sys].[dm_exec_query_stats] AS qs WHERE [qs].[query_hash]=@QueryHash AND NOT EXISTS(SELECT 1 FROM [#Candidate] c WHERE [c].[PlanHandle]=[qs].[plan_handle] AND COALESCE([c].[StatementStartOffset],-1)=COALESCE([qs].[statement_start_offset],-1))
            ORDER BY [qs].[total_worker_time] DESC;
        DELETE [c] FROM [#Candidate] AS c WHERE [c].[CandidateId] NOT IN(SELECT TOP (@EffectiveMaxAnalyseobjekte) [CandidateId] FROM [#Candidate] ORDER BY [CandidateId]);
        SELECT @RowCount=COUNT_BIG(*) FROM [#Candidate];SET @Detail=CASE WHEN @RowCount=0 THEN N'Kein passender Planhandle sichtbar.' ELSE N'Kandidaten erfolgreich ermittelt.' END;
    END TRY
    BEGIN CATCH
    ...
    BEGIN TRY
        ...
        FROM [#Candidate] AS c CROSS APPLY sys.dm_exec_plan_attributes([c].[PlanHandle]) AS pa;
    ...
    BEGIN TRY
        ...
        FROM [#Candidate] AS c OUTER APPLY sys.dm_exec_query_plan([c].[PlanHandle]) AS qp;
    END TRY BEGIN CATCH INSERT [#Plans] SELECT [CandidateId],'COMPILE_XML','ERROR_HANDLED',NULL,NULL,NULL,NULL,NULL,ERROR_NUMBER(),ERROR_MESSAGE() FROM [#Candidate];SET @IsPartial=1;SET @StatusCode='PARTIAL';END CATCH;
    ...
    BEGIN TRY
        ...
        FROM [#Candidate] AS c OUTER APPLY sys.dm_exec_text_query_plan([c].[PlanHandle],COALESCE([c].[StatementStartOffset],0),COALESCE([c].[StatementEndOffset],-1)) AS tp;
    END TRY BEGIN CATCH INSERT [#Plans] SELECT [CandidateId],'COMPILE_TEXT','ERROR_HANDLED',NULL,NULL,NULL,NULL,NULL,ERROR_NUMBER(),ERROR_MESSAGE() FROM [#Candidate];SET @IsPartial=1;SET @StatusCode='PARTIAL';END CATCH;
    ...
    BEGIN TRY
        ...
        FROM [#Candidate] AS c OUTER APPLY sys.dm_exec_query_plan_stats([c].[PlanHandle]) AS qp;
    END TRY BEGIN CATCH INSERT [#Plans] SELECT [CandidateId],'LAST_ACTUAL_XML','ERROR_HANDLED',NULL,NULL,NULL,NULL,NULL,ERROR_NUMBER(),ERROR_MESSAGE() FROM [#Candidate];SET @IsPartial=1;SET @StatusCode='PARTIAL';END CATCH;

    IF @StatusCode IN('AVAILABLE','PARTIAL') AND @MitLivePlan=1
    BEGIN TRY
        IF @SingleSessionId IS NULL
            INSERT [#Plans] VALUES(NULL,'LIVE_XML','INVALID_PARAMETER',NULL,NULL,NULL,NULL,NULL,NULL,N'@MitLivePlan erfordert genau eine Session in @SessionIds.');
        ELSE
            INSERT [#Plans]
            ...
            LEFT JOIN [#Candidate] AS c ON [c].[SessionId]=[qx].[session_id] AND ([c].[RequestId]=[qx].[request_id] OR [c].[RequestId] IS NULL);
    END TRY BEGIN CATCH INSERT [#Plans] VALUES(NULL,'LIVE_XML','ERROR_HANDLED',NULL,NULL,NULL,NULL,NULL,ERROR_NUMBER(),ERROR_MESSAGE());SET @IsPartial=1;SET @StatusCode='PARTIAL';END CATCH;
...
END;
    ...
    SELECT [c].[CandidateId],[c].[SessionId],[c].[RequestId],[c].[PlanHandle],[c].[SqlHandle],[c].[QueryHash],[c].[QueryPlanHash],[c].[StatementStartOffset],[c].[StatementEndOffset],[c].[CreationTime],[c].[LastExecutionTime],[c].[ExecutionCount],
           ...
           CASE WHEN @MaxSqlTextZeichen IS NULL OR @MaxSqlTextZeichen = 0 THEN [st].[text] ELSE LEFT([st].[text], @MaxSqlTextZeichen) END,[st].[dbid],DB_NAME([st].[dbid]),[st].[objectid]
    FROM [#Candidate] AS [c]
    ...
    BEGIN
        ...
        BEGIN
            SELECT * FROM [#CandidatesOutput] ORDER BY [CandidateId];
            IF @MitPlanAttributes=1 SELECT * FROM [#Attributes] ORDER BY [CandidateId],[AttributeName];
            IF @MitCompilePlan=1 OR @MitTextPlan=1 OR @MitLastActualPlan=1 OR @MitLivePlan=1 SELECT * FROM [#Plans] ORDER BY [CandidateId],[SourceType];
        END
        ELSE
        BEGIN
            SELECT N'Plan-Kandidat' AS [Ergebnis],[CandidateId] AS [Kandidat],[SessionId] AS [Session],[RequestId] AS [Request],[SqlTextDatabaseName] AS [Datenbank],[QueryHash] AS [Query Hash],[QueryPlanHash] AS [Plan Hash],[ExecutionCount] AS [Ausführungen],[LastExecutionTime] AS [letzte Ausführung],[SessionId] AS [Session SQL],[StatementText] AS [Statement],[BatchText] AS [Batch] FROM [#CandidatesOutput] ORDER BY [CandidateId];
            IF @MitPlanAttributes=1 SELECT N'Planattribut' AS [Ergebnis],[CandidateId] AS [Kandidat],[AttributeName] AS [Attribut],[AttributeValue] AS [Wert],[IsCacheKey] AS [Cache-Key] FROM [#Attributes] ORDER BY [CandidateId],[AttributeName];
            IF @MitCompilePlan=1 OR @MitTextPlan=1 OR @MitLastActualPlan=1 OR @MitLivePlan=1 SELECT N'Planinhalt' AS [Ergebnis],[CandidateId] AS [Kandidat],[SourceType] AS [Quelle],[StatusCode] AS [Status],[DatabaseId] AS [Datenbank-ID],[ObjectId] AS [Objekt-ID],[ErrorMessage] AS [Hinweis],[QueryPlanXml] AS [Plan XML],[QueryPlanText] AS [Plan Text] FROM [#Plans] ORDER BY [CandidateId],[SourceType];
        END;
    END;
    IF @JsonErzeugen=1
    BEGIN
