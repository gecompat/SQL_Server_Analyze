...
BEGIN
    ...
    CREATE TABLE [#Raw]([SourceType] varchar(20) NOT NULL,[TimestampUtc] datetime2(7) NULL,[FileName] nvarchar(260) NULL,[FileOffset] bigint NULL,[EventXml] xml NULL);
    ...
    BEGIN
        ...
        BEGIN
            ...
            INSERT [#SourceStatus] VALUES('CATALOG',NULL,NULL,@StatusCode,NULL,@ErrorMessage,
    ...
    BEGIN
        ...
        BEGIN
            ...
            INSERT [#SourceStatus] VALUES('EVENT_FILE',@ResolvedSessionName,NULL,@StatusCode,NULL,@ErrorMessage,N'Keine Datei gelesen.');
        END
        ELSE
        BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            INSERT [#SourceStatus] VALUES('RING_BUFFER',@ResolvedSessionName,NULL,@StatusCode,NULL,@ErrorMessage,N'Targetdaten nicht gelesen.');
        END
        ELSE
        BEGIN
            BEGIN TRY
                ...
                BEGIN
                    ...
                    INSERT [#SourceStatus] VALUES('RING_BUFFER',@ResolvedSessionName,NULL,@StatusCode,NULL,@ErrorMessage,N'Session gestoppt oder kein Ringbuffer.');
                END
                ELSE
                BEGIN
                    INSERT [#Raw]
                    ...
                    INSERT [#SourceStatus] VALUES('RING_BUFFER',@ResolvedSessionName,NULL,'AVAILABLE',NULL,NULL,N'Ringbuffer bewusst gelesen; Target-Flush möglich.');
                END;
            END TRY
            BEGIN CATCH
                SET @StatusCode=CASE WHEN ERROR_NUMBER() IN(229,262,297,300,371) THEN 'DENIED_PERMISSION' WHEN ERROR_NUMBER()=1222 THEN 'TIMEOUT' ELSE 'ERROR_HANDLED' END;
    ...
    IF @StatusCode='AVAILABLE'
    BEGIN
        INSERT [#Reports]([SourceType],[ReportTimeUtc],[FileName],[FileOffset],[ReportXml])
        ...
        SELECT @RowCount=COUNT_BIG(*) FROM [#Reports];
        IF @RowCount=0
...
END;
    ...
    CREATE TABLE [#ReportSummary]([ReportId] int,[SourceType] varchar(20),[ReportTimeUtc] datetime2(7),[FileName] nvarchar(260),[FileOffset] bigint,[MonitorLoop] bigint NULL,[BlockedSessionId] int NULL,[BlockingSessionId] int NULL,[WaitTimeMs] bigint NULL,[DatabaseId] int NULL,[DatabaseName] nvarchar(256),[WaitResource] nvarchar(1024),[RequestedLockMode] nvarchar(64),[ReportXml] xml NULL);
    CREATE TABLE [#BlockedProcesses]([ReportId] int,[ReportTimeUtc] datetime2(7),[SessionId] int NULL,[ExecutionContextId] int NULL,[ProcessStatus] nvarchar(128),[WaitResource] nvarchar(1024),[WaitTimeMs] bigint NULL,[LockMode] nvarchar(64),[DatabaseId] int NULL,[DatabaseName] nvarchar(256),[ClientApplication] nvarchar(512),[HostName] nvarchar(512),[LoginName] nvarchar(512),[InputBuffer] nvarchar(4000),[ProcessXml] xml NULL);
    CREATE TABLE [#BlockingProcesses]([ReportId] int,[ReportTimeUtc] datetime2(7),[SessionId] int NULL,[ExecutionContextId] int NULL,[ProcessStatus] nvarchar(128),[WaitResource] nvarchar(1024),[WaitTimeMs] bigint NULL,[LockMode] nvarchar(64),[DatabaseId] int NULL,[DatabaseName] nvarchar(256),[ClientApplication] nvarchar(512),[HostName] nvarchar(512),[LoginName] nvarchar(512),[InputBuffer] nvarchar(4000),[ProcessXml] xml NULL);
    INSERT [#ReportSummary] SELECT [r].[ReportId],[r].[SourceType],[r].[ReportTimeUtc],[r].[FileName],[r].[FileOffset],TRY_CONVERT(bigint,[r].[ReportXml].value('(/blocked-process-report/@monitorLoop)[1]','nvarchar(64)')),TRY_CONVERT(int,[r].[ReportXml].value('(/blocked-process-report/blocked-process/process/@spid)[1]','nvarchar(32)')),TRY_CONVERT(int,[r].[ReportXml].value('(/blocked-process-report/blocking-process/process/@spid)[1]','nvarchar(32)')),TRY_CONVERT(bigint,[r].[ReportXml].value('(/blocked-process-report/blocked-process/process/@waittime)[1]','nvarchar(64)')),TRY_CONVERT(int,[r].[ReportXml].value('(/blocked-process-report/blocked-process/process/@currentdb)[1]','nvarchar(32)')),[r].[ReportXml].value('(/blocked-process-report/blocked-process/process/@currentdbname)[1]','nvarchar(256)'),[r].[ReportXml].value('(/blocked-process-report/blocked-process/process/@waitresource)[1]','nvarchar(1024)'),[r].[ReportXml].value('(/blocked-process-report/blocked-process/process/@lockMode)[1]','nvarchar(64)'),CASE WHEN @MitReportXml=1 THEN [r].[ReportXml] END FROM [#Reports] [r];
    INSERT [#BlockedProcesses] SELECT [r].[ReportId],[r].[ReportTimeUtc],TRY_CONVERT(int,[p].[n].value('(@spid)[1]','nvarchar(32)')),TRY_CONVERT(int,[p].[n].value('(@ecid)[1]','nvarchar(32)')),[p].[n].value('(@status)[1]','nvarchar(128)'),[p].[n].value('(@waitresource)[1]','nvarchar(1024)'),TRY_CONVERT(bigint,[p].[n].value('(@waittime)[1]','nvarchar(64)')),[p].[n].value('(@lockMode)[1]','nvarchar(64)'),TRY_CONVERT(int,[p].[n].value('(@currentdb)[1]','nvarchar(32)')),[p].[n].value('(@currentdbname)[1]','nvarchar(256)'),[p].[n].value('(@clientapp)[1]','nvarchar(512)'),[p].[n].value('(@hostname)[1]','nvarchar(512)'),[p].[n].value('(@loginname)[1]','nvarchar(512)'),[p].[n].value('(inputbuf/text())[1]','nvarchar(4000)'),CASE WHEN @MitProcessXml=1 THEN [p].[n].query('.') END FROM [#Reports] [r] CROSS APPLY [r].[ReportXml].nodes('/blocked-process-report/blocked-process/process') [p]([n]);
    INSERT [#BlockingProcesses] SELECT [r].[ReportId],[r].[ReportTimeUtc],TRY_CONVERT(int,[p].[n].value('(@spid)[1]','nvarchar(32)')),TRY_CONVERT(int,[p].[n].value('(@ecid)[1]','nvarchar(32)')),[p].[n].value('(@status)[1]','nvarchar(128)'),[p].[n].value('(@waitresource)[1]','nvarchar(1024)'),TRY_CONVERT(bigint,[p].[n].value('(@waittime)[1]','nvarchar(64)')),[p].[n].value('(@lockMode)[1]','nvarchar(64)'),TRY_CONVERT(int,[p].[n].value('(@currentdb)[1]','nvarchar(32)')),[p].[n].value('(@currentdbname)[1]','nvarchar(256)'),[p].[n].value('(@clientapp)[1]','nvarchar(512)'),[p].[n].value('(@hostname)[1]','nvarchar(512)'),[p].[n].value('(@loginname)[1]','nvarchar(512)'),[p].[n].value('(inputbuf/text())[1]','nvarchar(4000)'),CASE WHEN @MitProcessXml=1 THEN [p].[n].query('.') END FROM [#Reports] [r] CROSS APPLY [r].[ReportXml].nodes('/blocked-process-report/blocking-process/process') [p]([n]);
