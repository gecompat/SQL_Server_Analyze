...
BEGIN
    ...
    BEGIN
        ...
        SET @Json=CONCAT(N'{"meta":',COALESCE(@Meta,N'{}'),N',"audits":',(SELECT TOP(@Limit) * FROM [#AuditConfigurationAnalysis_Audits] WHERE @NurProblematisch=0 OR [FindingSeverity]<>'INFO' ORDER BY [AuditName] FOR JSON PATH,INCLUDE_NULL_VALUES),N',"serverSpecifications":',(SELECT TOP(@Limit) * FROM [#AuditConfigurationAnalysis_ServerSpecifications] WHERE @NurProblematisch=0 OR [FindingSeverity]<>'INFO' ORDER BY [SpecificationName] FOR JSON PATH,INCLUDE_NULL_VALUES),N',"databaseSpecifications":',(SELECT TOP(@Limit) * FROM [#AuditConfigurationAnalysis_DatabaseSpecifications] WHERE @NurProblematisch=0 OR [FindingSeverity]<>'INFO' ORDER BY [DatabaseName],[SpecificationName] FOR JSON PATH,INCLUDE_NULL_VALUES),N',"sourceStatus":',(SELECT * FROM [#AuditConfigurationAnalysis_SourceStatus] ORDER BY [SourceName] FOR JSON PATH,INCLUDE_NULL_VALUES),N',"warnings":',(SELECT * FROM [#AuditConfigurationAnalysis_Warnings] ORDER BY [WarningScope],[DatabaseName],[AuditName] FOR JSON PATH,INCLUDE_NULL_VALUES),N'}');
    END;
    IF @OutputMode='RAW'
    BEGIN
