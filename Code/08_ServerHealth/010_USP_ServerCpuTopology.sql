...
BEGIN
 ...
 SET LOCK_TIMEOUT 0;
 BEGIN TRY
  INSERT [#I] SELECT [cpu_count],[scheduler_count],[hyperthread_ratio],[socket_count],[cores_per_socket],[numa_node_count],[softnuma_configuration_desc],[sqlserver_start_time],[affinity_type_desc] FROM [sys].[dm_os_sys_info];
  INSERT [#Sch] SELECT [parent_node_id],[status],COUNT_BIG(*),SUM(CONVERT(bigint,CASE WHEN [status]='VISIBLE ONLINE' THEN 1 ELSE 0 END)),SUM(CONVERT(bigint,[is_online])),SUM(CONVERT(bigint,[current_tasks_count])),SUM(CONVERT(bigint,[runnable_tasks_count])),SUM(CONVERT(bigint,[active_workers_count])),SUM(CONVERT(bigint,[load_factor])),
