...
AS
BEGIN
 SET NOCOUNT ON;SET @Json=NULL;DECLARE @ResultSetArtNormalisiert varchar(16)=UPPER(LTRIM(RTRIM(COALESCE(@ResultSetArt,''))));IF @Hilfe=1 BEGIN PRINT N'monitor.USP_TraceFlags';RETURN;END;
 ...
 CREATE TABLE [#X]([TraceFlag] int,[Status] int,[GlobalFlag] int,[SessionFlag] int);
 SET LOCK_TIMEOUT 0;BEGIN TRY INSERT [#X] EXEC(N'DBCC TRACESTATUS(-1) WITH NO_INFOMSGS;');END TRY BEGIN CATCH SELECT @S=CASE WHEN ERROR_NUMBER()IN(229,297,300,371)THEN'DENIED_PERMISSION'ELSE'ERROR_HANDLED'END,@P=1,@E=ERROR_NUMBER(),@M=ERROR_MESSAGE();END CATCH;
 ...
 IF @ResultSetArtNormalisiert<>'NONE' BEGIN SELECT CAST('2.0' AS varchar(16)) [ContractVersion],@T [CollectionTimeUtc],N'monitor.USP_TraceFlags' [ModuleName],@S [StatusCode],@P [IsPartial],@E [ErrorNumber],@M [ErrorMessage];IF @ResultSetArtNormalisiert='RAW' BEGIN SELECT * FROM [#X] ORDER BY [TraceFlag]; END ELSE BEGIN SELECT N'traceFlags' [Ergebnis],[x].* FROM [#X] [x] ORDER BY [TraceFlag]; END;END;
 IF @JsonErzeugen=1 BEGIN DECLARE @Meta nvarchar(max)=(SELECT N'TraceFlags' [resultName],1 [schemaVersion],@T [generatedAtUtc],@S [statusCode],@P [isPartial],@E [errorNumber],@M [errorMessage] FOR JSON PATH,WITHOUT_ARRAY_WRAPPER,INCLUDE_NULL_VALUES),@J0 nvarchar(max)=(SELECT * FROM [#X] ORDER BY [TraceFlag] FOR JSON PATH,INCLUDE_NULL_VALUES);SET @Json=CONCAT(N'{"meta":',COALESCE(@Meta,N'{}'),N',"traceFlags":',COALESCE(@J0,N'[]'),N',"warnings":[]}');END;
END;
GO
