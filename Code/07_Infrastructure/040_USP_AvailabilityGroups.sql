...
BEGIN
 ...
 IF @StatusCode='AVAILABLE' BEGIN TRY
  ...
  ELSE BEGIN
   INSERT [#R] SELECT TOP (@EffectiveMaxZeilen) [ag].[name],[ar].[replica_server_name],[rs].[is_local],[rs].[role_desc],[rs].[operational_state_desc],[rs].[connected_state_desc],[rs].[recovery_health_desc],[rs].[synchronization_health_desc],[ar].[availability_mode_desc],[ar].[failover_mode_desc],[ar].[session_timeout],[ar].[endpoint_url],[ar].[primary_role_allow_connections_desc],[ar].[secondary_role_allow_connections_desc],[ar].[read_only_routing_url] FROM [sys].[availability_groups] ag JOIN [sys].[availability_replicas] ar ON [ar].[group_id]=[ag].[group_id] LEFT JOIN [sys].[dm_hadr_availability_replica_states] rs ON [rs].[replica_id]=[ar].[replica_id] ORDER BY [ag].[name],[ar].[replica_server_name];
   INSERT [#D] SELECT TOP (@EffectiveMaxZeilen) [ag].[name],[ar].[replica_server_name],DB_NAME([drs].[database_id]),[drs].[is_local],sys.fn_hadr_is_primary_replica(DB_NAME([drs].[database_id])),[drs].[synchronization_state_desc],[drs].[synchronization_health_desc],[drs].[database_state_desc],[drs].[is_suspended],[drs].[suspend_reason_desc],[drs].[log_send_queue_size],[drs].[log_send_rate],CONVERT(decimal(19,2),[drs].[log_send_queue_size]*1.0/NULLIF([drs].[log_send_rate],0)),[drs].[redo_queue_size],[drs].[redo_rate],CONVERT(decimal(19,2),[drs].[redo_queue_size]*1.0/NULLIF([drs].[redo_rate],0)),[drs].[last_sent_time],[drs].[last_received_time],[drs].[last_hardened_time],[drs].[last_redone_time],[drs].[secondary_lag_seconds] FROM [sys].[dm_hadr_database_replica_states] drs JOIN [sys].[availability_replicas] ar ON [ar].[replica_id]=[drs].[replica_id] JOIN [sys].[availability_groups] ag ON [ag].[group_id]=[drs].[group_id] ORDER BY [ag].[name],DB_NAME([drs].[database_id]),[ar].[replica_server_name];
   INSERT [#L] SELECT TOP (@EffectiveMaxZeilen) [ag].[name],[l].[dns_name],[l].[port],[l].[is_conformant],[ip].[ip_address],[ip].[ip_subnet_mask],[ip].[network_subnet_ip],[ip].[network_subnet_prefix_length],[ip].[state_desc] FROM [sys].[availability_group_listeners] l JOIN [sys].[availability_groups] ag ON [ag].[group_id]=[l].[group_id] LEFT JOIN [sys].[availability_group_listener_ip_addresses] ip ON [ip].[listener_id]=[l].[listener_id] ORDER BY [ag].[name],[l].[dns_name],[ip].[ip_address];
   IF @MitRouting=1 INSERT [#Route] SELECT TOP (@EffectiveMaxZeilen) [ag].[name],[ar].[replica_server_name],[rl].[routing_priority],[ar2].[replica_server_name] FROM [sys].[availability_read_only_routing_lists] rl JOIN [sys].[availability_replicas] ar ON [ar].[replica_id]=[rl].[replica_id] JOIN [sys].[availability_replicas] ar2 ON [ar2].[replica_id]=[rl].[read_only_replica_id] JOIN [sys].[availability_groups] ag ON [ag].[group_id]=[ar].[group_id] ORDER BY [ag].[name],[ar].[replica_server_name],[rl].[routing_priority];
  END
 ...
 BEGIN
  ...
  BEGIN
   SELECT * FROM [#R] ORDER BY [AgName],[ReplicaServerName];
   SELECT * FROM [#D] ORDER BY [AgName],[DatabaseName],[ReplicaServerName];
   SELECT * FROM [#L] ORDER BY [AgName],[ListenerDnsName],[IpAddress];
   SELECT * FROM [#Route] ORDER BY [AgName],[ReplicaServerName],[RoutingPriority];
  END
  ELSE
  BEGIN
   SELECT N'Availability-Group-Replikat' AS [Ergebnis],[x].* FROM [#R] AS [x] ORDER BY [AgName],[ReplicaServerName];
   SELECT N'Availability-Group-Datenbank' AS [Ergebnis],[x].*,CONCAT(CONVERT(decimal(19,2),[LogSendQueueKb]/1024.0),N' MB') AS [Send-Queue],CONCAT(CONVERT(decimal(19,2),[RedoQueueKb]/1024.0),N' MB') AS [Redo-Queue] FROM [#D] AS [x] ORDER BY [AgName],[DatabaseName],[ReplicaServerName];
   SELECT N'Availability-Group-Listener' AS [Ergebnis],[x].* FROM [#L] AS [x] ORDER BY [AgName],[ListenerDnsName],[IpAddress];
   SELECT N'Read-Only-Routing' AS [Ergebnis],[x].* FROM [#Route] AS [x] ORDER BY [AgName],[ReplicaServerName],[RoutingPriority];
  END
 END;
 IF @JsonErzeugen=1
 BEGIN
  ...
  DECLARE @ReplicasJson nvarchar(max)=(SELECT * FROM [#R] ORDER BY [AgName],[ReplicaServerName] FOR JSON PATH,INCLUDE_NULL_VALUES);
  DECLARE @DatabasesJson nvarchar(max)=(SELECT * FROM [#D] ORDER BY [AgName],[DatabaseName],[ReplicaServerName] FOR JSON PATH,INCLUDE_NULL_VALUES);
  DECLARE @ListenersJson nvarchar(max)=(SELECT * FROM [#L] ORDER BY [AgName],[ListenerDnsName],[IpAddress] FOR JSON PATH,INCLUDE_NULL_VALUES);
