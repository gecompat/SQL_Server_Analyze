...
BEGIN
 ...
 BEGIN
  ...
  IF OBJECT_ID(N'msdb.dbo.log_shipping_primary_databases') IS NULL
     OR OBJECT_ID(N'msdb.dbo.log_shipping_monitor_primary') IS NULL
     OR OBJECT_ID(N'msdb.dbo.log_shipping_secondary_databases') IS NULL
     OR OBJECT_ID(N'msdb.dbo.log_shipping_monitor_secondary') IS NULL
  BEGIN
 ...
 BEGIN
  ...
  BEGIN SELECT * FROM [#P] ORDER BY [PrimaryDatabase];SELECT * FROM [#S] ORDER BY [SecondaryDatabase];END
  ELSE
  BEGIN
   SELECT N'Log-Shipping Primary' AS [Ergebnis],[x].*,CASE WHEN [BackupAgeMinutes] IS NULL THEN N'kein Zeitpunkt' ELSE CONCAT([BackupAgeMinutes],N' min') END AS [Backup-Alter] FROM [#P] AS [x] ORDER BY [PrimaryDatabase];
   SELECT N'Log-Shipping Secondary' AS [Ergebnis],[x].*,CASE WHEN [RestoreAgeMinutes] IS NULL THEN N'kein Zeitpunkt' ELSE CONCAT([RestoreAgeMinutes],N' min') END AS [Restore-Alter] FROM [#S] AS [x] ORDER BY [SecondaryDatabase];
  END
 END;
 IF @JsonErzeugen=1
 BEGIN
  ...
  DECLARE @PrimaryJson nvarchar(max)=(SELECT * FROM [#P] ORDER BY [PrimaryDatabase] FOR JSON PATH,INCLUDE_NULL_VALUES);
